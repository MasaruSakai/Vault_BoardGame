---
okf_version: "0.2"
---

# アーカムホラー 第3版 OKFバンドル

Sources内の画像をもとにした、アーカムホラー 第3版の日本語ルール知識バンドル。OKF v0.2の概念文書、YAML frontmatter、標準Markdownリンクで構成している。

## はじめに

* [ゲーム概要](game-overview.md) - ゲームの目的、人数、ラウンドの全体像。
* [内容物](components.md) - ボード、カード、トークン、ダイスの一覧。
* [ゲームの準備](setup.md) - シナリオ選択から初期配置までの手順。

## ルール

* [ラウンドとアクション](rounds-and-actions.md) - 4つのフェイズ、各アクション、能力判定。
* [モンスターと遭遇](encounters-and-monsters.md) - モンスター、交戦、遭遇カードの処理。
* [神話フェイズと異常現象](mythic-phase.md) - 神話トークン、破滅、異常遭遇。
* [探索者とカード](investigators-and-cards.md) - 探索者、役割、助力カード、状態。

## リファレンス

* [公式エラッタ](errata.md) - アークライト公式の訂正一覧とOKFへの反映先。
* [用語リファレンス](reference-glossary.md) - リファレンスガイドの主要用語とルール番号。
* [早見表](quick-reference.md) - 神話トークンと遭遇アイコンの即時参照。
* [公式FAQ・エラッタ整理（日本語）](official-faq-ja.md) - Fantasy Flight Games公式FAQ Update 9/18/20の全項目を日本語で整理。
* [BGGルールフォーラム整理（日本語）](bgg-rules-forum-ja.md) - BoardGameGeekのRulesフォーラム8ページ、781スレッドを対象にした日本語の補足整理。
* [BGG戦略フォーラム整理（日本語）](bgg-strategy-forum-ja.md) - BoardGameGeekのStrategyフォーラム30スレッドを対象にした日本語の戦略整理。
* [Official FAQ and Errata Digest (English)](official-faq-en.md) - English digest of the official FAQ Update 9/18/20.
* [BGG Rules Forum Digest (English)](bgg-rules-forum-en.md) - English digest of the selected rules discussions.
* [BGG Strategy Forum Digest (English)](bgg-strategy-forum-en.md) - English digest of the Strategy forum discussions.

* [完全読み起こし](references/full-transcription.md) - 40ページをページ順に整理した全文ノート。
* [原典ページ対応表](source-page-map.md) - OKF文書と原典画像の対応。
* [原典画像](references/source-pages/index.md) - Sourcesから複製した40枚の画像。
* [更新履歴](log.md) - このバンドルの変更履歴。

## コミュニティ

* [BGGコミュニティフォーラム整理（日本語）](bgg-community-forums-ja.md) - General、Sessions、Variantsの一般議論、プレイ記録、ハウスルール、ファン制作を整理。公式ルールのリファレンスではない。
* [BGG Community Forums Digest (English)](bgg-community-forums-en.md) - English digest of the General, Sessions, and Variants forums as community material rather than rules reference.
